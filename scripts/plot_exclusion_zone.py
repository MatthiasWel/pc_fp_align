...
def plot(df):
    ...
    comparision[f'delta_{metric}'] = comparision[f'{metric}_y'] - comparision[f'{metric}_x']
    comparision[f'delta_{metric}_abs'] = np.abs(comparision[f'delta_{metric}'])
    comparision[f'delta_{metric}_rel']= comparision[f'delta_{metric}_abs'] / np.abs(comparision[f'{metric}_y'])

    comparision[f'delta_cka_abs'] = np.abs(comparision['alignment_pc_fp_x'] - comparision['full_alignment_pc_fp_x'])
    
    sns.scatterplot(
        comparision,
        y=f'delta_{metric}_abs',
        x=f'full_alignment_pc_fp_x',
        hue=f'{metric}_y',
        ax=axs[0],
    )
    ...
    x_ticks, y_ticks, x_min, x_max, y_min, y_max, delta_x, delta_y = get_all_ticks(
        comparision[f'full_alignment_pc_fp_x'].values, 
    ...
    ax.set_xlim(x_min - offset_scale * delta_x / n_spaces, 
                x_max + offset_scale * delta_x / n_spaces)
    ax.set_ylim(y_min - offset_scale * delta_y / n_spaces,  
    ...
    # plot 2
    x = comparision[f'full_alignment_pc_fp_x'].values
    ...
    heatmap = original.pivot(index='x_percent', columns='y_percent', values='pvalue').values
    ...
    for n, ax in enumerate(axs.flat):
        ax
        ax.text(-0.15, 1.015, '(' + string.ascii_lowercase[n]  + ')', transform=ax.transAxes, #  
                size=fontsize, weight='bold')

    plt.tight_layout()
    plt.savefig(f"performance_delta_alignment.pdf")
    plt.show()

if __name__ == '__main__':
    df = pd.concat(
    [
        pd.read_csv('/data/shared/exchange/mwelsch/fp_pc_align/results/performance_2025-04-22_15:25:54.csv'), # adme
        pd.read_csv('/data/shared/exchange/mwelsch/fp_pc_align/results/performance_2025-04-23_10:56:29.csv'), # hts
        pd.read_csv('/data/shared/exchange/mwelsch/fp_pc_align/results/performance_2025-04-23_11:49:40.csv'), # tox
        pd.read_csv('/data/shared/exchange/mwelsch/fp_pc_align/results/performance_2025-04-23_15:24:44.csv'), # tox21
        pd.read_csv('/data/shared/exchange/mwelsch/fp_pc_align/results/performance_2025-04-25_12:01:46.csv')  # toxcast 
    ]
