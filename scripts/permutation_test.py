...
def test_at_thresholds(x, y, x_percent, y_percent):
    ...
    binom_stat = stats.binomtest(exclude_zone, n=len(x), p=expected_prob, alternative='less')
    ...
    for sim_nr in range(n_simulations):
        ...
        for x_percent in np.arange(0,1,0.01):
            for y_percent in np.arange(0,1,0.01):
                ...
                x_thresh, y_thresh, pvalue = test_at_thresholds(x, y_shuffled, x_percent, y_percent)
                simulation.append((x_percent, y_percent, pvalue))

        simulation = pd.DataFrame(simulation, columns=['x_percent', 'y_percent', 'pvalue'])
        heatmap_sim = simulation.pivot(index='x_percent', columns='y_percent', values='pvalue').values
        signif_mask_sim = heatmap_sim < threshold
        cluster_sim, n_clusters_sim = label(signif_mask_sim)
        cluster_sizes_sim = np.array([np.sum(cluster_sim == i) for i in range(1, n_clusters_sim + 1)])
        simuation_result.append(max(cluster_sizes_sim))

    

    print(
        (np.sum(max_cluster > simuation_result) + 1) / (n_simulations + 1)
    )
