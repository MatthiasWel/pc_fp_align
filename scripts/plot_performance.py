...
def plot(df):
    ...
    pc = df[(df.feature_type == 'PC')][['dataset',alignment_measure, metric, 'test_set_size', 'test_set_balance']]
    ...
    full_aggregate = pc.merge(
        fp, 
        on=['dataset', alignment_measure], 
        suffixes=('_PC', '_FP')
    ).merge(
        pcfp, 
        on=['dataset', alignment_measure], 
    ...
    axs[0].set_xlim(x_min - 0 * delta_x / n_spaces, 
                x_max + 0 * delta_x / n_spaces)
    axs[0].set_ylim(y_min - offset_scale * delta_y / n_spaces,  
    ...
    for col  in [col for col in test_full_aggregate.columns if 'mcc' in col]:
        ...
        for bootstrap in range(1000):
            ...
            bootstraps.append((bootstrap, col, slope, intercept, r_value, p_value, std_err))

    res_boot = pd.DataFrame(bootstraps, columns=('bootstrap', 'col', 'slope', 'intercept', 'r_value', 'p_value', 'std_err'))
    ...
    sns.heatmap(heat, xticklabels=ticklabels, yticklabels=ticklabels, ax=axs[1], square=True, vmin=0, vmax=1)
    ...
    plt.tight_layout()
    plt.savefig(f"performance_and_alignment.pdf")
    plt.show()
